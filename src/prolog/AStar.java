...
public class AStar {
	...
	public static AStarPath getPath(IVector2D Destination) {
		return AStar.getPath(new IVector2D(Singletons.heroPosition.x, Singletons.heroPosition.y),
				Singletons.heroDirection, Destination);
	}

	public static AStarPath getPath(IVector2D Origin, int OriginDirection, IVector2D Destination) {
		if (AStar.debugMode == true) {
			System.out.println("AStar Started: Origin: " + Origin.x + " , " + Origin.y + " | Destiny: " + Destination.x
					+ " , " + Destination.y + " | Direction: " + OriginDirection);
		}
		...
		while (tCell != dCell) {
			if (AStar.debugMode == true) {
				System.out.println("AStar Position: Origin: " + tCell.position.x + " , " + tCell.position.y
						+ " | Direction: " + tCell.ASData.direction);
			}
	...
	private static void addFrontier(ArrayList<Cell> frontier, Cell center, Cell destiny) {
		...
		if (nei != null && (nei.type == CellType.CLEAN ||nei.type == CellType.WATER || nei.type == CellType.GAS || nei.frontier))
	...
	private static void addCellToList(ArrayList<Cell> list, Cell cell, Cell predecessor, Cell finalDestination,
			int cellRelativeDirection) {
		// Cells to avoid:
		if (cell == null || cell.ASData.type != 0)
			return;
		if (cell.discovered == false && cell.frontier == false && cell != finalDestination)
			return;
		...
		if (AStar.debugMode == true) {
			System.out.println("ASCell: " + cell.position.x + "," + cell.position.y + " | " + cell.ASData.costToEnter
					+ " , " + cell.ASData.heuristicFinalCost);
		}
